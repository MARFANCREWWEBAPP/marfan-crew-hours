...
window.__MARFAN_SAFE_VERSION = '3.0.0';
...
// ---------- MARFAN 3 CONTROL DIARIO REAL UI ----------
window.MARFAN_VERSION='3.0.0';
(function(){
if(document.getElementById('m3-style')) return;
const st=document.createElement('style');st.id='m3-style';st.textContent='.m3-card{background:#fff;border:1px solid #e5e5ea;border-radius:20px;padding:18px;margin:14px 0;box-shadow:0 12px 30px rgba(0,0,0,.05)}.m3-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:12px;margin:12px 0}.m3-kpi{background:#fff;border:1px solid #e5e5ea;border-radius:18px;padding:16px}.m3-kpi small{display:block;color:#6e6e73;text-transform:uppercase;font-size:11px;font-weight:900}.m3-kpi b{font-size:24px}.m3-row{display:flex;justify-content:space-between;align-items:center;gap:12px;border-bottom:1px solid #eee;padding:12px 0}.m3-pill{display:inline-block;border-radius:999px;padding:4px 10px;font-size:12px;font-weight:900;background:#f2f2f7}.m3-ok{background:#e8f8ee;color:#137333}.m3-warn{background:#fff7e6;color:#8a5a00}.m3-bad{background:#ffe8e6;color:#b00020}.m3-btn{background:#000;color:#fff;border:none;border-radius:14px;padding:10px 14px;font-weight:900;cursor:pointer;margin:4px}@media(max-width:900px){.m3-grid{grid-template-columns:1fr 1fr}.m3-row{display:block}}';document.head.appendChild(st);
})();
function m3Content(){return document.getElementById('content')||document.querySelector('main')||document.body}
function m3Title(t){try{const h=document.querySelector('main h1,.main h1,h1');if(h)h.textContent=t;document.title='Marfan Crew · '+t}catch(e){}}
async function m3Json(url){try{const r=await fetch(url);return await r.json()}catch(e){return{ok:false,error:e.message}}}
function m3Kpi(l,v){return `<div class="m3-kpi"><small>${l}</small><b>${v}</b></div>`}
async function viewControlDiarioReal3(date){
  m3Title('Control Diario Real');
  const c=m3Content(), selected=date||new Date().toISOString().slice(0,10), d=await m3Json('/api/3/control-diario?date='+encodeURIComponent(selected)), cards=d.cards||{};
  c.innerHTML=`<div class="m3-card"><h2>Control Diario Real</h2><p class="muted">Eventos del día, personal asignado, fichados, pendientes e incidencias.</p><input type="date" id="m3-date" value="${selected}" style="padding:10px;border-radius:12px;border:1px solid #ddd"><button class="m3-btn" onclick="viewControlDiarioReal3(document.getElementById('m3-date').value)">Actualizar</button></div>
  <div class="m3-grid">${m3Kpi('Eventos hoy',cards.events_today||0)}${m3Kpi('Asignados',cards.workers_assigned||0)}${m3Kpi('Fichados',cards.workers_checked_in||0)}${m3Kpi('Pendientes',cards.workers_pending||0)}${m3Kpi('Incidencias',cards.incidents||0)}${m3Kpi('Sin personal',cards.events_without_staff||0)}${m3Kpi('Sin jefe',cards.events_without_team_lead||0)}</div>
  <div class="m3-card"><h3>Torre diaria de eventos</h3>${(d.events||[]).length?(d.events||[]).map(item=>{const e=item.event||{},cls=item.status==='completo'?'m3-ok':item.status==='sin_personal'?'m3-bad':'m3-warn';return `<div class="m3-card"><div class="m3-row"><div><h3>${e.name||e.title||'Evento'}</h3><p class="muted">${e.event_date||e.date||''} · ${e.start_time||e.planned_start||''}-${e.end_time||e.planned_end||''}</p><span class="m3-pill ${cls}">${item.status}</span> ${item.team_lead?`<span class="m3-pill">Jefe: ${item.team_lead.name}</span>`:`<span class="m3-pill m3-bad">Sin jefe</span>`}</div><div><b>${item.checked_in}/${item.assigned}</b><br><small>fichados</small></div></div><details><summary>Ver operarios</summary>${(item.assignments||[]).map(a=>`<div class="m3-row"><div><b>${a.checked_in?'✅':'⏳'} ${a.name}</b><br><span class="muted">${a.role||'Operario'} · ${a.planned_start||''}-${a.planned_end||''}</span></div><div>${a.phone?`<a class="m3-btn" href="tel:${a.phone}">Llamar</a>`:''}</div></div>`).join('')||'<p>Sin operarios asignados.</p>'}</details></div>`}).join(''):'<p>No hay eventos para esta fecha.</p>'}</div>
  <div class="m3-card"><h3>Incidencias del día</h3>${(d.incidents||[]).length?(d.incidents||[]).map(i=>`<div class="m3-pill m3-bad">${i.created_at||''} · ${i.type||''} · ${i.description||''}</div>`).join('<br>'):'<p>Sin incidencias.</p>'}</div>`;
}
window.viewControlDiarioReal3=viewControlDiarioReal3;
console.log('Marfan Crew 3 Control Diario Real cargado');
// ---------- END MARFAN 3 UI ----------
