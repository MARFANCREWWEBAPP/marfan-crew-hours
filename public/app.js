    ...
    <div class="v6264-card">
      ...
      ${(d.operators||[]).length ? (d.operators||[]).map(u=>`
        <div class="v6264-row">
          ...
          <button class="v6264-btn" onclick="v6266OpenOperatorPortal('${u.id}')">Ver portal</button>
...
async function viewOperariosProV6265(){
  ...
  <div class="v6265-card"><h3>Listado de operarios</h3>${workers.map(w=>`<div class="v6265-row"><div><b>${w.name}</b><br><span class="muted">${w.phone||''} ${w.email||''} · ${w.role||''}</span><br><small>Servicios: ${w.stats.assignments_total||0} · Docs: ${w.stats.documents_total||0} · Caducados: ${w.stats.documents_expired||0}</small></div><button class="v6265-btn" onclick="v6266OpenOperatorPortal('${w.id}')">Expediente</button></div>`).join('')||'<p>No hay operarios.</p>'}</div>
    ...
    main.innerHTML = `
      ...
      <div class="v6263-card">
        ...
        ${(d.operators||[]).length ? (d.operators||[]).map(u=>`
          ...
          <button class="v6263-btn" onclick="v6266OpenOperatorPortal('${u.id}')">Ver portal</button></div>
...
document.addEventListener('click', function(ev){
  ...
  if(match && match[1]){
    ev.preventDefault();
    ev.stopPropagation();
    window.v6266OpenOperatorPortal(match[1]);
  }
}, true);
