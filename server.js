...
function v601EnsureEventColumns(){
  const existing = db.prepare('PRAGMA table_info(events)').all().map(c=>c.name);
  const add = (name,type)=>{
    if(!existing.includes(name)){
      try{ db.prepare(`ALTER TABLE events ADD COLUMN "${name}" ${type}`).run(); }catch(e){}
...
try{ v601EnsureEventColumns(); }catch(e){}

app.get('/api/event-v601-edit', requireAdmin, (req,res)=>{
  try{
    v601EnsureEventColumns();
...
app.post('/api/event-v601-edit', requireAdmin, (req,res)=>{
  try{
    v601EnsureEventColumns();
    const id = Number(req.query.id);
    if(!id) return res.status(400).json({ok:false,error:'ID de evento inválido'});
