...
app.get('/api/v627-data-status', requireAdmin, (req,res)=>{
  try{
    ...
    res.json({ok:true, version: '62.68.0', data_dir:dataDir, db_path:dbPath, exists, size});
  }catch(e){
    res.status(500).json({ok:false,error:e.message});
  }
});
...
function v6223ExportUsersJson(){
  ...
  try{
    ...
    f.writeFileSync(out, JSON.stringify({version: '62.68.0', created_at:new Date().toISOString(), users}, null, 2));
...
try {
  app.get('/api/v6254/health', (req,res)=>{
    res.json({ok:true, version:'62.68.0', message:'Visual Solapamientos activo'});
  });
...
try {
  app.get('/api/v6255/health',(req,res)=>{
    res.json({ok:true,version:'62.68.0',message:'Jefe equipo + firma + bloqueo activo'});
  });
...
try {
  app.get('/api/v6258/health',(req,res)=>{
    res.json({ok:true,version:'62.68.0',message:'Centro Control Live activo'});
  });
...
try {
  ...
  app.get('/api/v6259/health',(req,res)=>{
    res.json({ok:true,version:'62.68.0',message:'Dashboard CEO + Inteligencia Operativa activo'});
  });
...
try {
  app.get('/api/v6260/health',(req,res)=>{
    res.json({ok:true,version:'62.68.0',message:'Centro Operativo Live activo'});
  });
...
try {
  ...
  app.get('/api/v6261/health',(req,res)=>{
    res.json({ok:true,version:'62.68.0',message:'Disponibilidad + Planificador activo'});
  });
...
try {
  app.get('/api/v6263/health',(req,res)=>res.json({ok:true,version:'62.68.0',message:'Integración real UI activa'}));
...
try {
  app.get('/api/v6264/health',(req,res)=>res.json({ok:true,version:'62.68.0',message:'UI Core Fix activo'}));
} catch(e) {}
// ---------- END V62.64 ----------
...
try {
  v6265EnsureTables();
  app.get('/api/v6265/health',(req,res)=>res.json({ok:true,version:'62.68.0',message:'Operarios Pro + Expediente RRHH activo'}));
...
try {
  app.get('/api/v6266/health',(req,res)=>res.json({ok:true,version:'62.68.0',message:'Fix Portal Operario activo'}));
} catch(e) {}
// ---------- END V62.66 ----------
...
try {
  app.get('/api/v6267/health',(req,res)=>res.json({ok:true,version:'62.68.0',message:'Fix Portal Safari + ID Seguro activo'}));
...
// ---------- V62.68 RAILWAY DEPENDENCY FIX ----------
try {
  app.get('/api/v6268/health',(req,res)=>{
    let expressOk = false;
    try { require('express'); expressOk = true; } catch(e) {}
    res.json({ok:true,version:'62.68.0',message:'Railway Dependency Fix activo',express:expressOk,node:process.version});
  });
} catch(e) {}
// ---------- END V62.68 ----------
