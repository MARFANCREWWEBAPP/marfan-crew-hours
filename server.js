...
app.get('/api/v627-data-status', requireAdmin, (req,res)=>{
  try{
    ...
    res.json({ok:true, version: '62.34.0', data_dir:dataDir, db_path:dbPath, exists, size});
  }catch(e){
    res.status(500).json({ok:false,error:e.message});
  }
});
...
function v6223ExportUsersJson(){
  ...
  try{
    ...
    f.writeFileSync(out, JSON.stringify({version: '62.34.0', created_at:new Date().toISOString(), users}, null, 2));
...
app.listen(PORT, '0.0.0.0', () => {
  console.log(`Marfan Crew Hours V62.34 Stop Calendar AutoSync Popup Restore listening on port ${PORT}`);
});
